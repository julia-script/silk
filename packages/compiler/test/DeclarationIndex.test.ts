...
it('indexes mixed struct and function declarations in one canonical namespace', () => {
  const index = collect('root', [
    [
      'root',
...
it('retains duplicate and damaged struct fields without losing later fields', () => {
  const index = collect('root', [
...
it('diagnoses private exposure and inline recursive struct components canonically', () => {
  const exposedSource =
    'struct Hidden {}\n' +
    'pub struct Public { pub hidden: Hidden private: Hidden }\n' +
    'pub fn reveal(value: Hidden) -> Hidden { return value }'
  const exposed = collect('root', [['root', exposedSource]])
  assert.deepEqual(
...
it('resolves forward nominal fields into a canonical acyclic dependency set', () => {
  const index = collect('geometry', [
    [
      'geometry',
