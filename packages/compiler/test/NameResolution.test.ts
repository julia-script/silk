...
it.effect('keeps repeated imports valid without hiding parser recovery diagnostics', () =>
  Effect.gen(function* () {
    ...
    assert.strictEqual(Analysis.lookupName(duplicate, 'root', 'One')._tag, 'Namespace')
    assert.strictEqual(Analysis.lookupName(duplicate, 'root', 'Again')._tag, 'Namespace')
    assert.deepEqual(
...
pub fn main() -> i32 { return one() }`
    ...
    assert.deepEqual(Analysis.diagnostics(self), [])
    assert.strictEqual(Analysis.lookupName(self, 'root', 'One')._tag, 'Namespace')
    assert.strictEqual(Analysis.lookupName(self, 'root', 'one')._tag, 'Resolved')
  }),
)
