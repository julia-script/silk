...
pub fn main() -> i32 { return 42 }`
    ...
    const service = module?.services.find(
      (candidate) => candidate.name._tag === 'Present' && candidate.name.spelling === 'ByteDuplex',
    )
    assert.deepEqual(Analysis.diagnostics(snapshot), [])
    assert.deepEqual(
    ...
    const tlsModule = index.modules.find((candidate) => candidate.module === 'silk/tls_connection')
