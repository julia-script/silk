...
it('retains source field order while elaborating canonical construction and projection', () => {
  const self = snapshot()
  ...
  assert.strictEqual(makeHir === undefined ? undefined : Hir.returned(makeHir)._tag, 'Construct')
...
it('evaluates initializers in source order before constructing in declaration order', () => {
...
it('plans canonical aggregate lanes and evaluates whole-value calls and projections', () => {
  const self = snapshot('wasm32-unknown-unknown')
...
it.effect('keeps LLVM, WebAssembly, and evaluation in aggregate parity', () =>
  Effect.gen(function* () {
    const native = snapshot('aarch64-apple-darwin')
    const wasm = snapshot('wasm32-unknown-unknown')
...
pub fn main() -> I32 { return consume(end()) }`
    const self = Analysis.ofSource('struct-values/empty', ascii(text), 'wasm32-unknown-unknown')
    const layout = Analysis.layoutOf(self)
    assert.strictEqual(layout._tag, 'Available')
    if (layout._tag !== 'Available') return
...
it('keeps invalid construction, projection, and partial moves phase-owned', () => {
  const invalid = Analysis.ofSource(
...
it('allows external factory construction while refusing external raw literals', () => {
...
pub fn main() -> I32 { let token = Model.make(1) return token.kind }`),
  ...
  const valid = Analysis.make({ rootModule: 'app/Main', sources })
