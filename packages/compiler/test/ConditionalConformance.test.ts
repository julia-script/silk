...
pub fn main() -> i32 {
  ...
  return run bridge(&mut provider, ConcreteHandler {})
    ...
    const runner = mir.functions.find((fn) => fn.id.name === 'acquire$effect$-1')
    assert.isDefined(runner)
    if (runner === undefined) return
    assert.notInclude(JSON.stringify(runner), 'InterfaceOperationCall')
    const witnessCalls = MirVerification.operations(runner).filter(
      (operation) => operation._tag === 'Call' && operation.target.name === 'Context.use',
    )
    assert.lengthOf(witnessCalls, 1)
    assert.isTrue(
      witnessCalls.every((call) => call.typeArguments.every(Type.isRuntimeConcreteGenericArgument)),
    )
    const acquireCall = hir?.functions
      .flatMap((fn) => fn.statements)
      .flatMap(Hir.statementExpressions)
      .flatMap(Hir.expressionTree)
      .find((expression) => expression._tag === 'Call' && expression.target.name === 'acquire')
    assert.strictEqual(acquireCall?._tag, 'Call')
    if (acquireCall?._tag !== 'Call') return
    assert.deepEqual(
