...
it('catalogs empty and nested structs before reachability and reuses their exact entries', () => {
  const snapshot = Analysis.ofSource(
    'layout/catalog',
    ascii(`struct Empty {}
...
pub fn main() -> Outer { return 42 }`),
  ...
  assert.strictEqual(Layout.entry(plan.value, Type.nominal('layout/catalog', 'Unused')), undefined)
})

it('retains unavailable fields, cycles, and transitive dependencies without harming peers', () => {
  const snapshot = Analysis.ofSource(
    'layout/unavailable',
...
it('encodes nominal catalogs identically for repeated inputs and preserves target identity', () => {
  const source = ascii('struct Pair { left: I32 right: Bool }\npub fn main() -> I32 { return 42 }')
  const first = Analysis.ofSource('layout/repeat', source, 'aarch64-apple-darwin')
  const second = Analysis.ofSource('layout/repeat', source, 'aarch64-apple-darwin')
  const wasm = Analysis.ofSource('layout/repeat', source, 'wasm32-unknown-unknown')
...
it('reports malformed aggregate facts and divergence from the catalog', () => {
  const snapshot = Analysis.ofSource(
  ...
  assert.strictEqual(selected._tag, 'Available')
  if (selected._tag !== 'Available') return
  const pair = Layout.catalogEntry(selected.value, Type.nominal('layout/verify-aggregate', 'Pair'))
  assert.strictEqual(pair?._tag, 'LayoutEntry')
