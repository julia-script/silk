...
? &mut Allocator | &mut MonotonicClock {
  ...
  if count != 4 || !equal(&body, b"Wiki") || !(run request.bodyComplete()) { return 31 }
...
! ServerError | OutOfMemoryError
? &mut Allocator | &mut MonotonicClock {
  if !(run request.bodyComplete()) { return 81 }
  let entries: [Header<'static>; 0] = []
  let headers = match move Headers.make(&entries, valueLimits()) {
...
? &mut Allocator | &mut MonotonicClock {
  let mut body: [u8; 4] = [0, 0, 0, 0]
  let count = run readSome(&mut request.*, &mut body, Option.none<Instant>())
  if count != 4 || !equal(&body, b"Wiki") || !(run request.bodyComplete()) { return 120 }
  let entries: [Header<'static>; 0] = []
  let headers = match move Headers.make(&entries, valueLimits()) {
...
? &mut Allocator | &mut MonotonicClock {
  ...
  if !(run request.bodyComplete()) { return 131 }
  return run handleZeroBody(move request)
}
...
? &mut Allocator | &mut MonotonicClock {
  ...
  let resumed = run Effect.result(request.bodyComplete())
...
! ServerError | OutOfMemoryError
? &mut Allocator | &mut MonotonicClock {
  if !(run request.bodyComplete()) { return 201 }
  let entries: [Header<'static>; 0] = []
  let headers = match move Headers.make(&entries, valueLimits()) {
...
! ServerError | OutOfMemoryError
? &mut Allocator | &mut MonotonicClock {
  if (run request.bodyComplete()) { return 211 }
  let entries: [Header<'static>; 0] = []
  let headers = match move Headers.make(&entries, valueLimits()) {
