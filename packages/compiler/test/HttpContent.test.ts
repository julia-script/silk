...
it.effect(
  'keeps content-reader borrows and typed recovery valid in focused runtime MIR',
  () =>
    Effect.gen(function* () {
      const module = 'http-content/runtime-mir'
      const configuration = AnalysisFixture.configuration(module, 'x86_64-unknown-linux-gnu', [
        'portableProgram',
      ])
      const snapshot = yield* Analysis.makeRealized({
        root: SourceFile.make(module, encoder.encode(httpContentAcceptanceSource)),
        configuration,
      }).pipe(Effect.provide(SourceResolver.empty))
      assert.deepEqual(Analysis.diagnostics(snapshot), [])
      assert.deepEqual(MirVerification.verify(Analysis.loweredMir(snapshot)), [])
...
it.effect(
  'keeps provided Effect.result success shapes distinct across recovery families',
  () =>
    Effect.gen(function* () {
