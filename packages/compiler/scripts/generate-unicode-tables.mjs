...
for (const line of lines(await read('UnicodeData.subset.txt'))) {
  ...
  else if (components.length === 2) pairs.push({ scalar, first: components[0], second: components[1] })
...
const fullLength = (scalar, seen = new Set()) => {
  ...
  if (seen.has(scalar)) throw new Error(`Canonical decomposition of U+${scalar.toString(16)} cycles`)
...
const combiningRuns = () => {
  ...
  for (const [scalar, klass] of ordered) {
    ...
    if (last !== undefined && last.klass === klass && last.start + last.count === scalar && last.count < 256) {
      last.count += 1
      continue
    }
...
const emit = () => `// Generated by scripts/generate-unicode-tables.mjs from the vendored Unicode ${version} database.
...
if (check) {
  ...
  for (const [name, count, bytes] of report) {
    process.stdout.write(`${name.padEnd(26)} ${String(count).padStart(5)} entries ${String(bytes).padStart(6)} bytes\n`)
  }
