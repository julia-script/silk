...
const program = Effect.gen(function* () {
  ...
  const output = path.resolve(
    yield* Config.string('SILK_SUPPLY_OUTPUT').pipe(
      Config.withDefault('.scratch/standard-streams'),
    ),
  )
  ...
  if (headers === undefined)
    return yield* new ConformanceError({ message: `Missing stream header pins: ${target}` })
  for (const [header, expected] of Object.entries(headers)) {
    ...
    if (actual !== expected)
      return yield* new ConformanceError({ message: `Unpinned stream header: ${header}` })
  }
  ...
  for (const optimization of ['none', 'speed']) {
    ...
    const lane = yield* NativeToolchain.withBuildScope(
      ...
      Effect.fnUntraced(function* (scope) {
        const snapshot = yield* Analysis.makeRealized({
          root: 'stream-conformance/root',
          configuration: { profile: { ...input, artifact: 'object', entry: { kind: 'none' } } },
        }).pipe(
          Effect.provide(
            SourceResolver.overlay([SourceFile.make('stream-conformance/root', source)]).pipe(
        ...
        const expectedForeign = [
          target.includes('apple') ? '__error' : '__errno_location',
          'read',
          'write',
        ]
        if (
          ...
          return yield* new ConformanceError({
            message: 'Stream fixture retained an unexpected foreign or runtime dependency',
          })
        ...
        if (
          ...
          !inspection.stdout.includes('read') ||
          !inspection.stdout.includes('write')
        )
          return yield* new ConformanceError({
            message: 'Object inspection did not verify architecture and stream relocation',
          })
        let execution
        if (target.includes('apple')) {
          const cwd = yield* fs.makeTempDirectoryScoped({ prefix: 'silk-stream-execution-' })
    ...
    yield* Console.log(
      `${target} ${optimization}: source stream providers verified against independent headers and deterministic foreign calls`,
    )
  }
