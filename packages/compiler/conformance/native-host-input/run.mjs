...
const program = Effect.gen(function* () {
  ...
  const output = path.resolve(
    yield* Config.string('SILK_SUPPLY_OUTPUT').pipe(
      Config.withDefault('.scratch/native-host-input'),
    ),
  )
  ...
  const pins = yield* Schema.decodeEffect(
    ...
    yield* fs.readFileString(
      path.join(
        directory,
        '../../../../openspec/changes/complete-native-runtime-migration/host-input-supplies.json',
      ),
    ),
  )
  const headers = pins.headers[target]
  if (headers === undefined)
    return yield* new ConformanceError({ message: `Missing process-input header pins: ${target}` })
  for (const [header, expected] of Object.entries(headers)) {
    ...
    if (actual !== expected)
      return yield* new ConformanceError({ message: `Unpinned process-input header: ${header}` })
  }
  ...
  const cases = [
    { name: 'inputs', source, receiver },
    {
      name: 'faults',
      source,
      receiver: yield* fs.readFileString(path.join(directory, 'fault-receiver.c')),
    },
  ]
  ...
  for (const optimization of ['none', 'speed']) {
    ...
    for (const fixture of cases) {
      const lane = yield* NativeToolchain.withBuildScope(
        ...
        Effect.fnUntraced(function* (scope) {
          const snapshot = yield* Analysis.makeRealized({
            ...
            Effect.provide(
              SourceResolver.overlay([
                SourceFile.make('input-conformance/root', fixture.source),
          ...
          if (
            ...
            return yield* new ConformanceError({
              message: `Source input provider acquired a generated runtime dependency: ${artifact.nativeRuntimeSymbols.join(', ')}`,
            })
          ...
          const cCompilation = yield* PlatformSupplyResolver.query(
            ...
            'independent C process-input receiver',
          )
          ...
          if (
            ...
            !inspection.stdout.includes('getcwd') ||
          ...
          if (target.includes('apple')) {
            const cwd = yield* fs.makeTempDirectoryScoped({ prefix: 'silk-input-execution-' })
      ...
      yield* Console.log(`${target} ${optimization} ${fixture.name}: source process inputs passed`)
    }
  }
