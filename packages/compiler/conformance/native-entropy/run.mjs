...
const program = Effect.gen(function* () {
  ...
  const output = path.resolve(
    yield* Config.string('SILK_SUPPLY_OUTPUT').pipe(Config.withDefault('.scratch/native-entropy')),
  )
  ...
  const pins = yield* Schema.decodeEffect(
    ...
    yield* fs.readFileString(
      path.join(
        directory,
        '../../../../openspec/changes/source-owned-native-entropy/supplies.json',
      ),
    ),
  )
  const headers = pins.headers[target]
  if (headers === undefined)
    return yield* new ConformanceError({ message: `Missing entropy header pins: ${target}` })
  for (const [header, expected] of Object.entries(headers)) {
    ...
    if (actual !== expected)
      return yield* new ConformanceError({ message: `Unpinned entropy header: ${header}` })
  }
  ...
  for (const optimization of ['none', 'speed']) {
    ...
    const lane = yield* NativeToolchain.withBuildScope(
      ...
      Effect.fnUntraced(function* (scope) {
        ...
        const expectedForeign = target.includes('apple')
          ? ['arc4random_buf']
          : ['__errno_location', 'getrandom']
        if (
          artifact.foreignImports
            .map((value) => value.symbol)
            .sort()
            .join(',') !== expectedForeign.join(',') ||
          artifact.nativeRuntimeSymbols.length !== 0
        )
          return yield* new ConformanceError({
            message: 'Entropy fixture retained an unexpected foreign or runtime dependency',
          })
        ...
        const cCompilation = yield* PlatformSupplyResolver.query(
          ...
          'independent C entropy receiver',
        )
        ...
        if (
          ...
          !inspection.stdout.includes(target.includes('apple') ? 'arc4random_buf' : 'getrandom')
        )
          return yield* new ConformanceError({
            message: 'Object inspection did not verify architecture and entropy relocation',
          })
        let execution
        if (target.includes('apple')) {
          const cwd = yield* fs.makeTempDirectoryScoped({ prefix: 'silk-entropy-execution-' })
    ...
    yield* Console.log(
      `${target} ${optimization}: source entropy provider verified against independent headers and deterministic foreign calls`,
    )
  }
