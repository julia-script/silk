...
const program = Effect.gen(function* () {
  ...
  const output = path.resolve(
    yield* Config.string('SILK_SUPPLY_OUTPUT').pipe(Config.withDefault('.scratch/native-report')),
  )
  ...
  const pins = yield* Schema.decodeEffect(
    ...
    yield* fs.readFileString(
      path.join(
        directory,
        '../../../../openspec/changes/complete-native-runtime-migration/report-supplies.json',
      ),
    ),
  )
  const headers = pins.headers[target]
  if (headers === undefined)
    return yield* new ConformanceError({ message: `Missing report header pins: ${target}` })
  for (const [header, expected] of Object.entries(headers)) {
    ...
    if (actual !== expected)
      return yield* new ConformanceError({ message: `Unpinned report header: ${header}` })
  }
  ...
  const cases = [
    { name: 'formatting', source, receiver },
    {
      name: 'observer-cancel',
      source: yield* fs.readFile(path.join(directory, 'observer-cancel.silk')),
      receiver: yield* fs.readFileString(path.join(directory, 'cancel-receiver.c')),
    },
    {
      name: 'observer-suspended',
      source: yield* fs.readFile(path.join(directory, 'observer-suspended.silk')),
      receiver: yield* fs.readFileString(path.join(directory, 'observer-receiver.c')),
    },
    {
      name: 'observer',
      source: yield* fs.readFile(path.join(directory, 'observer.silk')),
      receiver: yield* fs.readFileString(path.join(directory, 'observer-receiver.c')),
    },
    {
      name: 'context',
      source: yield* fs.readFile(path.join(directory, 'context.silk')),
      receiver: yield* fs.readFileString(path.join(directory, 'context-receiver.c')),
    },
  ]
    ...
    for (const fixture of cases) {
      const lane = yield* NativeToolchain.withBuildScope(
        ...
        Effect.fnUntraced(function* (scope) {
          const snapshot = yield* Analysis.makeRealized({
                      ...
                      components: [
                        {
                          ...
                          bindings: ['create', 'acquire', 'release', 'destroy'].map(
                            (operation) => ({
                              operation,
            ...
            Effect.provide(
              SourceResolver.overlay([
                SourceFile.make('report-conformance/root', fixture.source),
          ...
          }).pipe(Effect.tapError((error) => Console.error(error.reason)))
          if (
            ...
            return yield* new ConformanceError({
              message: `Source report formatter acquired a generated runtime dependency: ${artifact.nativeRuntimeSymbols.join(', ')}`,
            })
          ...
          if (
            ...
            !inspection.stdout.includes('write')
          )
            return yield* new ConformanceError({
              message: 'Object inspection did not verify architecture and input relocation',
            })
          let execution
          if (target.includes('apple')) {
            const cwd = yield* fs.makeTempDirectoryScoped({ prefix: 'silk-report-execution-' })
      ...
      yield* Console.log(
        `${target} ${optimization} ${fixture.name}: source report formatting passed`,
      )
    }
  }
