...
export const effectValueType = (
  ...
  const environment = layout.effectEnvironments.find(
    (candidate) =>
      ...
      Type.equals(candidate.effect, requested),
  )
...
const effectEnvironmentsByIdentity = (
  ...
  return direct.length > 0 ? direct : success.length > 0 ? success : recovered
}
...
export const effectValueForCall = (
  ...
  const environment =
    exact.length === 1 ? exact.at(0) : contextual.length === 1 ? contextual.at(0) : undefined
