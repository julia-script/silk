...
const fixtureRoot = resolve(packageRoot, 'fixtures/declarations')
const moduleScript = resolve(packageRoot, 'scripts/declaration-module.mjs')
const command = process.argv[2] ?? 'verify'
const executable = (name, variable) =>
  Effect.runSync(Config.string(variable).pipe(Config.withDefault(name)))

if (command === 'regenerate') {
  const text = execFileSync(process.execPath, [moduleScript, 'text'])
  const bitcode = execFileSync(process.execPath, [moduleScript, 'bitcode'])
...
const temporary = mkdtempSync(resolve(tmpdir(), 'silk-effect-llvm-declarations-'))
try {
  const text = resolve(fixtureRoot, 'representative.ll')
  const directBitcode = resolve(temporary, 'direct.bc')
  writeFileSync(
    directBitcode,
    Buffer.from(readFileSync(resolve(fixtureRoot, 'representative.bc.hex'), 'utf8').trim(), 'hex'),
  )
