...
it(
  'cancels a same-document request while its exact revision is pending',
  { timeout: stdioTestTimeout },
  async () => {
...
it(
  'pulls only the final diagnostics through effects to valid repair',
  {
    timeout: stdioTestTimeout,
  },
  async () => {
...
it(
  'keeps a committed project live while another project accepts rapid slow edits',
  {
    timeout: stdioTestTimeout,
  },
  async () => {
...
it(
  'clears a failed version, recovers on the next edit, and shuts down after the defect',
  {
    timeout: stdioTestTimeout,
  },
  async () => {
...
it(
  'refreshes sibling documents when an imported module changes',
  { timeout: stdioTestTimeout },
  async () => {
    const client = connect()
    try {
...
it(
  'serves project-wide references and renames over real stdio',
  { timeout: stdioTestTimeout },
  async () => {
    const client = connect()
    try {
      ...
      const toolchainLocations = (await client.waitFor((message) =>
        response(message, 8),
      )) as Array<{
        uri: string
      }>
...
it(
  'offers a diagnostic edit as a quick fix over real stdio',
  { timeout: stdioTestTimeout },
  async () => {
    const client = connect()
    try {
...
it(
  'discovers, resolves, and rejects stale auto-import actions over real stdio',
  {
    timeout: stdioTestTimeout,
  },
  async () => {
...
it(
  'resolves a standard-library auto-import using Silk source-path spelling',
  {
    timeout: stdioTestTimeout,
  },
  async () => {
