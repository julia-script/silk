...
const project = (): string => {
  const root = mkdtempSync(join(tmpdir(), 'silk-doctest-'))
...
it.effect('refuses an input that is missing, malformed, or absent altogether', () =>
  Effect.gen(function* () {
    const root = project()
    try {
      assert.strictEqual(yield* Cli.run({ input: join(root, 'absent.json') }), 2)
      const malformed = join(root, 'malformed.json')
      writeFileSync(malformed, '{ not json')
      assert.strictEqual(yield* Cli.run({ input: malformed }), 2)
      assert.strictEqual(yield* Cli.run({}), 2)
      assert.strictEqual(yield* Cli.run({ stdlib: true, input: malformed }), 2)
    } finally {
      rmSync(root, { recursive: true, force: true })
    }
  }).pipe(Effect.provide(NodeServices.layer)),
