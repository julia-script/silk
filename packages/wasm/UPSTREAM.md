...
`@silklang/wasm` implements the WebAssembly specification directly; it ports no upstream
...
1. `pnpm --filter @silklang/wasm test` — unit tests plus committed-fixture comparison
   (byte-identical binary, character-identical text). No external tools; runs in CI.
2. `pnpm --filter @silklang/wasm parity:oracle` — oracle validation of every fixture binary,
...
3. `pnpm --filter @silklang/wasm fixtures:regenerate` — rebuilds committed fixtures after an
