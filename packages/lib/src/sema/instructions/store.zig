...
pub fn emit(block: *InstContext.Block, hir_inst_index: Hir.Inst.Index) !Sema.Instruction.Index {
    ...
    const pointer_inst_index = block.ctx.getInstructionByHirIndex(hir_inst.store.pointer);
    const pointer_inst = block.ctx.getInstruction(pointer_inst_index);
    const pointer_type = block.ctx.builder.unwrapPointerType(pointer_inst.typed_value.type) orelse std.debug.panic("unwrapped_pointer_inst is not a type {d}", .{pointer_inst_index});

    switch (pointer_type) {
        .complex => |complex| switch (block.ctx.builder.getComplexType(complex).data) {
            .@"struct" => {
                return try handleStoreStruct(block, hir_inst_index);
            },
            .array => {
                return try handleStoreArray(block, hir_inst_index);
            },
            else => |data| {
        ...
        .simple => |simple| switch (simple) {
            ...
            .void,
            => {
                const instruction_index = block.ctx.getInstructionByHirIndex(hir_inst.store.value);

                const value_inst_index = (try block.pushMaybeCastInstructionToType(
...
pub fn pushSetInstructionField(
    ...
    const field_init_inst_value = block.ctx.builder.getValue(field_init_inst.typed_value.value) orelse std.debug.panic("field_init_inst_value is not a number", .{});
    ...
    //     .data = .void,
    // });
    const get_element_pointer_inst_index = try block.appendInstruction(hir_inst_index, .{
    ...
    const store_inst_index = try block.appendInstruction(hir_inst_index, .{
        .op = .store,
        .typed_value = .{
            .type = Sema.Type.simple(.void),
            .value = Sema.Value.simple(.void),
    ...
    try maybeInline(block, store_inst_index);
    return store_inst_index;
}
pub fn handleStoreStruct(block: *InstContext.Block, hir_inst_index: Hir.Inst.Index) !Sema.Instruction.Index {
    const hir_inst = block.ctx.getHirInstruction(hir_inst_index);
    const pointer_inst_index = block.ctx.getInstructionByHirIndex(hir_inst.store.pointer);

    const value_inst_index = block.ctx.getInstructionByHirIndex(hir_inst.store.value);
    ...
    switch (value_inst.op) {
        .type_init => {
            ...
            for (list) |field_inst_index| {
                _ = try pushSetInstructionField(block, hir_inst_index, pointer_inst_index, field_inst_index);
            }
...
pub fn handleStoreArray(block: *InstContext.Block, hir_inst_index: Hir.Inst.Index) !Sema.Instruction.Index {
    const hir_inst = block.ctx.getHirInstruction(hir_inst_index);
    const pointer_inst_index = block.ctx.getInstructionByHirIndex(hir_inst.store.pointer);
    const pointer_inst = block.ctx.getInstruction(pointer_inst_index);
    ...
    switch (value_inst.op) {
        .array_init => {
            ...
            for (list, 0..) |item_inst_index, i| {
                ...
                const item_inst = try block.pushMaybeCastInstructionToType(hir_inst_index, item_inst_index, element_type) orelse item_inst_index;

                const store_inst = try block.appendInstruction(hir_inst_index, .{
                    .op = .store,
                    .typed_value = .{
                        .type = Sema.Type.simple(.void),
                        .value = Sema.Value.simple(.void),
                ...
                try maybeInline(block, store_inst);
        ...
        .constant => {
            const type_to_store = block.ctx.builder.unwrapPointerType(pointer_inst.typed_value.type) orelse std.debug.panic("type_to_store is not a type", .{});

            const value_to_store_inst_index = try block.pushMaybeCastInstructionToType(
                hir_inst_index,
                value_inst_index,
                type_to_store,
...
pub fn maybeInline(block: *InstContext.Block, inst_index: Sema.Instruction.Index) !void {
    ...
    var base_inst_value = block.ctx.getTypedValue(inst.data.operand_payload.operand);
    var value_inst_value = block.ctx.getTypedValue(inst.data.operand_payload.payload);
    ...
    const ptr = try block.ctx.builder.readNumberAsType(usize, base_inst_value);
    // ctx.markDead(inst.data.operand_payload.operand);
    // ctx.markDead(inst.data.operand_payload.payload);
    // ctx.markDead(inst_index);
    try block.ctx.builder.sema.memory.store(ptr, value_inst_value);
}
...
pub fn exec(ctx: *ExecContext, inst_index: Sema.Instruction.Index) !void {
    ...
    var base_inst_value = ctx.getInstruction(inst.data.operand_payload.operand).typed_value;
    var value_inst_value = ctx.getInstruction(inst.data.operand_payload.payload).typed_value;
