...
pub fn emit(block: *InstContext.Block, hir_inst_index: Hir.Inst.Index) !Sema.Instruction.Index {
    ...
    switch (hir_inst) {
        .number_literal => |ast_node| {
            const slice = entity.getHir().ast.getNodeSlice(ast_node.node);
            ...
            const value_index = try entity.builder.internValueData(.{ .bytes = bytes });
        ...
        .boolean_literal => |ast_node| {
            ...
            return block.appendInstruction(hir_inst_index, .{
                ...
                .typed_value = .{
                    ...
                    .value = try entity.builder.numberAsBytesValueKey(is_true),
                },
                .data = .void,
            });
        },
        .string_literal => |ast_node| {
            var slice = entity.getHir().ast.getNodeSlice(ast_node.node);
            if (slice[0] == '"') {
                slice = slice[1 .. slice.len - 1];
            }
            const ty = try entity.builder.internTypeData(.{ .array = .{
                .child = Sema.Type.simple(.bchar),
                .len = @intCast(slice.len),
            } });

            const pointer = try entity.builder.sema.memory.create(ty);
            entity.builder.sema.memory.storeAt([]const u8, pointer, slice);

            return block.appendInstruction(hir_inst_index, .{
                .op = .constant,
                .typed_value = .{
                    .type = ty,
                    .value = try entity.builder.numberAsBytesValueKey(pointer),
                },
                .data = .void,
            });
        },
        .char_literal => |ast_node| {
            ...
            return block.appendInstruction(hir_inst_index, .{
                ...
                .typed_value = .{
                    ...
                    .value = try entity.builder.numberAsBytesValueKey(char_float),
                },
                .data = .void,
            });
        },
