...
it('generates one longest-first TextMate rule for every committed literal form', () => {
  const literals = TextMate.grammar.patterns.filter((pattern) =>
    pattern.name?.startsWith('string.'),
  )
...
it('names every grammar keyword as a semantic keyword token', () => {
  const source = `${Object.values(TextMate.keywords).join(' ')}\n`
  const bytes = new TextEncoder().encode(source)
  const snapshot = Effect.runSync(Analysis.ofSource('main', bytes))
  const document = Document.make({
    uri: 'file:///main.silk',
    version: 1,
    workspace: 'project:/silk.toml',
    module: 'main',
    sourceRoot: '/',
    bytes,
  })
  const tokens = Document.semanticTokens(document, snapshot).data
  const keywordIndex = Document.semanticTokenTypes.indexOf('keyword')
  assert.notStrictEqual(keywordIndex, -1)
  assert.strictEqual(tokens.length / 5, Object.keys(TextMate.keywords).length)
  for (let index = 3; index < tokens.length; index += 5) {
    assert.strictEqual(tokens[index], keywordIndex)
  }
})
...
it('assigns keyword, numeric, and comment scopes via a TextMate tokenizer', async () => {
  ...
  assert.include(scopesAt('"text\\n"', 'text'), 'string.quoted.double.silk')
  assert.include(scopesAt('b"bytes"', 'bytes'), 'string.quoted.double.byte.silk')
  const multiline = '"""hé\n// not a comment\nreturn |> \\"\\"\\"\nend"""'
  assert.include(scopesAt(multiline, '// not a comment'), 'string.quoted.triple.silk')
