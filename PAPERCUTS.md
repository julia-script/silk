...
- 2026-09-25 · An enum payload for primitive semantic types made the unchanged M1 native test runner fail during bootstrap LLVM emission · Keep the already validated byte-backed primitive payload and compare its exact contents; coordinate a separate current-main emitter repair if enum layout becomes necessary · self-hosted compiler
