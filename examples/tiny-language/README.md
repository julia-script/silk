...
It imports only public `@silk-lang/llvm/*` package subpaths and is intentionally outside the
repository's pnpm workspace.

After `@silk-lang/llvm` is published, install and verify it with:
...
unpublished `@silk-lang/llvm` dependency from that temporary `package.json`, and then install the
tarball with `pnpm add /absolute/path/to/silk-lang-llvm-0.0.0.tgz --save-exact`. Deleting the npm
