...
It imports only public `@silklang/llvm/*` package subpaths and is intentionally outside the
repository's pnpm workspace.

After `@silklang/llvm` is published, install and verify it with:
...
unpublished `@silklang/llvm` dependency from that temporary `package.json`, and then install the
tarball with `pnpm add /absolute/path/to/silklang-llvm-0.0.0.tgz --save-exact`. Deleting the npm
