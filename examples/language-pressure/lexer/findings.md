...
| Standard library | `Vector.get` originally required `&mut Vector<T>` even for Copy reads, so observation consumed `Lexed`, destructured it, and rebound each vector as mutable. | `fingerprint`, `fingerprintVectors`, the `silk.vector` public `get` signature, and cross-engine `RawBuffer.read` evidence | Repaired after the stack VM independently confirmed the wall. `fingerprint` now shared-matches `Lexed`, and both nominal Copy vectors are read through `&Vector<T>` without moving storage, allocating, or changing cleanup. Structural-union reads remain a separate defect. |
...
parser port. Shared Vector reads are now complete for the evidence-backed non-union Copy boundary;
the structural-union copy repair remains separate.
