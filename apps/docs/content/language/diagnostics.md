...
| `SEM0171` | Stable code for positional construction with the wrong number of tuple elements. | `<type> expects <expected> tuple elements but received <actual>` |
| `SEM0172` | Stable code for using tuple syntax with a named struct or record syntax with a tuple. | `A contextual <expected> literal cannot construct <actual>` |
| `SEM0173` | Stable code for attempting to join distinct anonymous aggregate occurrences. | `Separate anonymous aggregate occurrences do not acquire a common type` |
| `SEM0174` | Stable code for attempting named-field construction of a positional aggregate. | `<type> is positional and cannot be constructed with named fields` |
