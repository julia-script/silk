...
An `impl` supplies every contract operation exactly once. It may define an operation body inline,
map the operation to an existing actor function, or mix those forms for different operations. An
inline operation is scoped to that conformance; it does not become a separately callable member of
the provider type.
...
The inline name must match the contract operation. A mapped implementation uses
`operation: Module.function`; its target may be an ordinary user-defined function or an Intrinsic
whose specialized signature satisfies the same contract. `silk.numeric` uses mappings to give one
...
Services use the same inline, mapped, and mixed conformance forms. The only additional service
privilege is eligibility to appear in an Effect requirement row. See the
[prescriptive conformance rules](../reference/requirements-and-services.md#serv-001--a-conformance-may-define-or-map-each-operation).
...
Borrows are written `&` and `&mut`. Direct borrow expressions exist only as call arguments and
parameters, so a standalone binding such as `let view = &values` is rejected. An ordinary function
with exactly one borrowed parameter may return a view proven to derive from that parameter. The
caller may bind, pass, reborrow, project, or transitively return that view while its source owner
remains live. The view cannot be stored in owned data or carried as an Effect success or failure.

The exactly-one-parameter rule makes the returned source unambiguous without named lifetimes. A
shared result may derive from a shared or exclusive parameter; an exclusive result requires an
exclusive parameter. A result derived from a local owner, temporary hidden inside the callee, or a
...
### 5.7 Explicit suspension

`Effect.suspend(child)` is an explicit stack-safety boundary for Effect execution. It preserves
the child's success, failure, and requirement channels exactly. A terminating recursive Effect
cycle uses bounded native and WebAssembly *machine* stack only when every possible cycle crosses a
suspension boundary. Merely declaring `effect fn` does not make recursion stack safe.

Suspension is not asynchronous execution. It does not park a task, run another task, choose a
scheduler, or make ordinary function recursion and recursive cleanup stack safe. Compiler-owned
execution-stack storage is finite; exhausting it is a fatal trap outside the typed failure channel.

See [Recursion and stack safety](./recursion.md) for the practical selection rule and
[Effects, failures, and services](./effects.md) for composition.

### 5.8 Fibers and cooperative scheduling

The shipped `silk.execution`, `silk.scheduler`, `silk.fiber`, and `silk.local_scheduler` modules
provide explicit single-threaded structured concurrency in ordinary Silk source. An application
constructs a scheduler provider and calls its entry operation; the language does not select one at
`main`.

`Fiber.forkChild` creates attached child work. `Fiber.await` or `Fiber.join` consumes the affine
Fiber handle. A parent that terminates cancels unfinished descendants before its outcome becomes
observable. The local scheduler selects ready work in deterministic FIFO order, and a task runs
until it completes, parks, or yields.

This is cooperative concurrency, not parallel execution: there are no threads, detached tasks,
preemption, or `async` syntax. See [Fibers and local scheduling](./fibers.md).
...
Parallel execution, multithreading, networking, a package registry, broad FFI, and self-hosting are
future work. Cooperative single-threaded Fibers are available through the explicit standard-library
scheduler described in [§5.8](#58-fibers-and-cooperative-scheduling).

## See also

- [Tutorial](./tutorial.md) — from `silk init` to a running program.
- [Ownership, borrowing, and cleanup](./ownership.md) — the practical lifetime model.
- [Effects, failures, and services](./effects.md) — the three-channel computation model.
- [Fibers and local scheduling](./fibers.md) — explicit cooperative structured concurrency.
- [Alpha status and supported targets](./alpha-status.md) — implemented scope, target matrix, and
