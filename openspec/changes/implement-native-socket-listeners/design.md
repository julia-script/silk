...
poll, and each GNU TCP pending-network error all route through the positive wait before retry. The same
...
set is accepted only after TCP accept and always waits before retry. Configuration, resource, permission,
