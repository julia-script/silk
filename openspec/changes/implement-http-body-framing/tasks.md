...
- [x] 1.2 Implement request and response receiving selection plus independent outgoing validation; verify the consolidated portable acceptance source covers HEAD, 1xx/204/304, CONNECT, 205, HTTP/1.0, conflicting fields, strict Transfer-Encoding, and duplicate/overflowing Content-Length with distinct runtime sentinels.
...
- [x] 2.4 Implement terminal single-message decoder failure, abandonment, and finite discard; verify later calls return InvalidState, bounded discard counts framing overhead, and a distinct message requires a fresh Selection/policy owner.
...
- [x] 4.2 Add focused compiler tests and one exported portable acceptance source; verify one shared analysis snapshot covers ownership and opacity contracts while the shared native corpus plus intended LLVM-to-Wasm leg cover selection, runtime framing, trailers, limits, encoding, and exact boundaries without a redundant per-feature native test.
