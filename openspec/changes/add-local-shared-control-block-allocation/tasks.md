...
- [x] 1.1 Add `sharedLayout<T>` and unsafe `sharedFromAllocation<T>` to the sealed intrinsic catalog
...
- [x] 1.2 Publish semantic call facts that connect the concrete `T` and target layout provenance across
...
- [x] 2.1 Plan one opaque control-block layout covering strong state, access state, private reclaim
...
- [x] 2.2 Add one consuming MIR initialization transition that accepts only the matching active
...
- [x] 2.3 Transfer self-contained reclaim authority into the control block without retaining allocator
...
- [x] 3.1 Add an ordinary-source construction fixture whose allocator rejects the exact layout and
      verify no initializer, core, or release obligation is created and `T` is cleaned exactly once
      by typed-failure cleanup.
- [x] 3.2 Audit semantic analysis, HIR, MIR, evaluation, and backend dispatch for actor-name checks;
