...
- [ ] 6.3 Record a direct security/correctness assessment of the exact committed issue diff against verified JUL-187 ancestry, fix valid findings, and repeat affected focused checks. Follow the user's no-new-review-agent-cycle instruction and explicitly distinguish this assessment from independent approval.
- [ ] 6.4 Record a direct `TEST_REVIEW.md` test-economics assessment of the committed issue diff, fix valid findings, and report measured evidence with timing limits. Do not claim independent approval or start another review-agent cycle.
- [ ] 6.5 Verify JUL-187's final merged head is an ancestor, regenerate affected artifacts, complete direct assessments and local focused verification, and verify exact-final-head CI passes the full suite, `pnpm check`, and release-candidate job.
