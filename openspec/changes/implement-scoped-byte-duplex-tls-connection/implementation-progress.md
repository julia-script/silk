...
TLS native execution, TLS Wasm execution, remaining local checks, and exact-head full CI are still pending. Workspace `pnpm typecheck` passed. The repaired native compilation completed semantic and ownership checking but exhausted both 4 GiB and 6 GiB heaps during instance discovery; this is not a passing runtime witness. Profiling found 179,259 path contexts for only 1,328 prepared instances, with repeated context keys around 22 KiB. Exact local interning now keeps those path-key atoms shared without changing recursion checks; the focused compile is being remeasured. The draft remains in progress, and no review or final handoff is claimed.
