...
- [ ] 1.1 Add `silk.http_server` public phases, limits, body/reuse policies, progress, and closed
...
- [ ] 1.2 Implement `withConnection` over one explicit concrete provider and bounded buffered
...
- [ ] 1.3 Implement serial `withRequest` head admission, parser reset, clean-end/truncation
...
- [ ] 2.1 Implement request framing selection plus streaming `readSome` and finite
...
- [ ] 2.2 Implement bounded Expect parsing and exactly-once automatic 100 output; verify mixed-case
...
- [ ] 2.3 Implement informational response validation and the shared finite budget; verify 100-199
...
- [ ] 2.4 Implement final-head normalization and serialization plus nested `ResponseWriter` body
...
- [ ] 3.1 Implement HTTP/1.0 and HTTP/1.1 persistence planning before output; verify close-token
...
- [ ] 3.2 Implement recoverable pre-output validation and terminal post-output failure/cancellation
...
- [ ] 3.3 Implement explicit `finishConnection` flush, write-shutdown, bounded-drain, and close-state
      ordering; verify end/deadline/drain-limit outcomes and that cancellation finalization performs only
      nonparking close.
- [ ] 3.4 Implement scoped generic Upgrade and CONNECT tunnel handoff; verify request/token/body and
...
- [ ] 4.1 Add `silk.http_server_native.serveNext` over scoped listener accept and the portable
...
- [ ] 4.2 Add one focused analysis file and one exported portable acceptance source, consolidating
...
- [ ] 4.3 Add the streaming HTTP server reference page; verify examples use public imports and the
...
- [ ] 4.4 Register both public modules and the shared native/Wasm/native-admission fixtures through
