...
explicit trailer finish that flushes the response before successful completion, retain no caller
input borrow, and distinguish bytes accepted by bounded server output from bytes accepted by the
provider. A buffering/provider failure after encoder
