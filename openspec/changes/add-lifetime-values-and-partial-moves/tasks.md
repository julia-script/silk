...
- [ ] 6.4 Run required milestone checks in order (pnpm typecheck, pnpm format:check, pnpm lint, pnpm test, pnpm check, and pnpm release:candidate for package contents/exports); record exact outcomes and distinguish pre-existing failures.
  ...
  Current milestone (2026-09-05, `179ad084`): workspace typecheck passes 18/18 tasks; formatting, lint and strict OpenSpec validation pass. The local workspace test run passed LSP, CLI and documentation tests, then stopped with 2,240 compiler tests passing and one failing suspended-owner native regression. CI independently reproduced that regression. All 16 native cases rejected by the preceding CI run pass focused execution after the captured-lifetime, callable-cleanup, composite-layout and masked-cleanup fixes. The suspended-owner regression now passes after liveness follows live references to their concrete storage roots; six focused suspension controls pass, retaining the genuine partial-owner rejection. All three Linux native CI shards at `179ad084` passed. CI also exposed an outdated release-candidate export list; adding the intended LifetimeElision API passes the focused packed-package check. Final milestone checks, `pnpm check` and full release-candidate validation remain pending. Logs: `/tmp/silk-lifetime-work/green-{typecheck,format,lint,test}.log`.
