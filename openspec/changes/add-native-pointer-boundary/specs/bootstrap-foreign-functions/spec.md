...
`float` and `double` classes; and all admitted qualified single/many and nullable/non-null data pointers for any pointee `T` as the C pointer
...
realized for that target. Classification SHALL consume a validated target description and SHALL preserve pointer qualifier contracts in semantic signature identity. Slices SHALL remain inadmissible as bare C pointers.
...
- **WHEN** a module declares `unsafe extern "C" fn allocateBytes(size: usize) -> ?[*]mut u8` and `unsafe extern "C" fn releaseBytes(pointer: ?[*]mut u8) -> ()`
