...
- [x] 1.1 Add target-neutral MIR operations for local-shared layout, initialization, clone, callback
...
- [x] 1.2 Encode deterministic MIR inspection data without actor names, addresses, or backend field
      offsets; verify committed goldens are byte-identical across repeated analysis.

## 2. Evaluator Oracle

- [x] 2.1 Model logical block identity, target layout, bounded strong count, independent access state,
...
- [x] 2.2 Emit bounded logical events for initialization, clone, access, conflict, restoration, decrement,
      payload cleanup, and release; verify nested conflict records no state change and last drop records
      cleanup before release.
- [x] 2.3 Add evaluator acceptance cases for sequential mutation, clone/non-last drop during access,
...
- [x] 3.1 Lower planned control-block storage and every verified transition in native LLVM using
...
- [x] 3.2 Lower the same operations in direct Wasm using its existing reclaim path; verify non-LIFO
      final release returns storage and no scheduler, collector, atomics, or background work appears.
- [x] 3.3 Verify both backends keep conflict observation non-mutating, restore access only after normal
...
- [x] 4.1 Add the local-shared programs to the designated native differential acceptance corpus and
      verify evaluator, native, and Wasm agree on results and logical ownership events without a
      per-feature native compile test.
- [x] 4.2 Cover the four nested access combinations, clone/drop during access, last cleanup, typed
...
- [x] 4.3 Unit-test overflow with a reduced private maximum and structurally verify compare-before-store
...
- [x] 4.4 Run `pnpm typecheck`, `pnpm exec biome check .`, `pnpm test`, and `pnpm check`; verify every
