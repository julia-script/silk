...
- [ ] 3.3 Publish one typed TIR body plus necessary indexed results, remove duplicate executable facts and syntax-backed rebinding, and migrate public/tooling terminology and goldens. The contract, representative artifacts, consumer map and step rationale are in `tir-contract.md`.
  ...
  - [ ] 3.3.5 Direct construction through a private body builder; delete `FunctionFact`, `StatementFact`, `ExpressionFact`, `TirLowering` and the fact half of `ResidualBody`. Landed: the fact half of `ResidualBody` is gone and working records are private to construction (`test/RecordBoundary.test.ts`). Open: analysis still builds records and `TirLowering` still builds nodes from them.
