...
- [ ] 1.1 Reserve `tuple` and add syntax kinds for named tuple declarations, positional tuple
...
- [ ] 1.2 Parse `tuple Point(T0, T1)`, `Point(v0, v1)`, `(v0, v1)`, `(v0,)`, `.{ field: value }`,
...
- [ ] 1.3 Add bounded recovery for missing tuple names, types, elements, record labels, colons,
...
- [ ] 1.4 Extend the formatter and syntax inspector for the new source forms without exposing
...
- [ ] 2.1 Add an `AggregateIdentity` actor for source and occurrence-generated nominal identities,
...
- [ ] 2.2 Generalize canonical aggregate members to a closed labeled-or-ordinal identity while
...
- [ ] 2.3 Index named tuple declarations as positional nominal aggregates and record generated
  ...
  those boundaries.
- [ ] 2.4 Add diagnostic catalog entries for tuple arity and position mismatches, incompatible
...
- [ ] 3.1 Add the narrow optional-expected-aggregate analysis path at explicit bindings, declared
...
- [ ] 3.2 Resolve named tuple construction, contextual tuple literals, and ordinal projections with
...
- [ ] 3.3 Resolve contextual `.{ ... }` literals through the existing struct authority,
...
- [ ] 3.4 Finalize each uncontextualized tuple or record literal once as an occurrence-nominal
  ...
  selected.
- [ ] 3.5 Enforce nominal boundaries for anonymous aggregates; verify separate same-shaped literals
  fail assignment, equality, and uncontextualized branch joining, while an explicit enclosing named
  type context makes both branches construct the same nominal type.

## 4. Erase aggregate sugar into the existing runtime pipeline

- [ ] 4.1 Elaborate every accepted named tuple, contextual literal, and anonymous aggregate into
  ...
  node.
- [ ] 4.2 Carry occurrence-generated nominal identities through generic substitution and runtime
...
- [ ] 4.3 Apply ordinary struct Copy evidence, moves, borrows, partial-move rejection, and recursive
...
- [ ] 4.4 Confirm layout, MIR, evaluator, LLVM, and WebAssembly consume only the generalized nominal
...
- [ ] 5.1 Extend semantic facts, deterministic encodings, hover, navigation, and contextual field
...
- [ ] 5.2 Update the prescriptive language reference for tuple declarations, positional literals and
...
- [ ] 5.3 Add shared `Analysis.evaluate` fixtures for function-call convenience, locally bound generic
...
- [ ] 5.4 Regenerate affected compiler and documentation artifacts, then run `pnpm typecheck`,
