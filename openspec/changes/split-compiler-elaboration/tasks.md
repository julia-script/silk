...
- [x] 1.1 Extract `ExpressionAnalysis.ts` (and pull the three inline branches into named functions)
...
- [x] 2.1 Add the parameterized `lowerStatements` helper and delete `hirEffectStatements`
- [x] 2.2 Route the `hirStatements` closure through it and verify determineism tests

## 3. Dedup borrow-id and access reduction

- [x] 3.1 Add `argumentBorrowId`/`loanEndsOf` and replace the six-plus inline copies
- [x] 3.2 Route all five access-reduction sites through `strongestEffectAccess`
