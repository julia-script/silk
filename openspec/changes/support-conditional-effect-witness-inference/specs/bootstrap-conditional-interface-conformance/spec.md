...
#### Scenario: Preserve exact Effect runner execution channels

- **WHEN** one lexical recovery combinator is instantiated for distinct success or failure families,
  with provider specialization and caller-local access or lifetime views
- **THEN** runner selection preserves the exact success, failure, and requirement channels, physical
  closure identity, and provider witness; equal ABI layouts alone never authorize reuse
- **AND** only exact selected capability, role, and requirement-access evidence may remove a
  requirement, while the physical runner body remains independent of caller-local proof metadata

#### Scenario: Preserve constructor capture contracts after provider selection

- **WHEN** a generic Effect constructor captures a service Effect replaced by a concrete provider
- **THEN** its MIR parameters retain the physical capture contracts published for that exact owner
  and parameter ordinal, without losing parameters or inventing storage lifetimes for row binders
