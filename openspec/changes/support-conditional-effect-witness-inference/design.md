...
### Separate Effect value views from runner execution contracts

A call retains its exact semantic Effect view, including access and invocation lifetimes. Runner
selection keys the exact success, failure row, and requirement row, together with the physical
source site, concrete owner, static arguments, and provider witness. Outer access and executable
lifetime proofs describe use of the closure rather than a different executable machine. A provided
runner is lowered from its physical base environment, never from caller-local proof metadata.
Provider subtraction must account for every removed requirement with exact capability, role, and
requirement access. Provisional service selection applies the same relation when a selected
...
implementation. Declaration-only witness dependency discovery has no invocation context. Symbolic
source conformance selections survive both ordinary calls and Effect construction until concrete
specialization checks the selected declaration again.
