...
- [ ] 9.3 Delete `Intrinsic.effectResult` and its analysis, HIR, MIR, evaluator, Wasm, LLVM, layout-discovery, and suspension-metadata support without replacement, and verify the intrinsic inventory plus repository searches contain no completed-outcome primitive or compatibility path.
