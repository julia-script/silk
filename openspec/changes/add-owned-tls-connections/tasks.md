...
- [x] 1.1 Revise the in-flight native-socket planning artifacts to remove the callback-only
...
- [x] 1.2 Expose owned resolved-TCP and pathname-Unix acquisition over the existing affine native
...
- [x] 2.1 Remove `ConnectionOptions.trust`, the now-unused `TrustLoadLimits` import, the
...
- [x] 2.2 Migrate every TLS connection caller and fixture to load, decode, combine, or copy one
...
- [x] 2.3 Update TLS connection documentation and executable examples to distinguish caller-owned
...
- [x] 3.1 Refactor the existing TLS connection pump to operate on private state plus an explicit
      ...
      `ByteDuplex` access.
- [x] 3.2 Implement guarded `authenticateOwned` so its guard begins after receiving `P` and one
...
- [x] 3.3 Extend `ConnectionOptions` with the optional external absolute deadline, clamp it to the
...
- [x] 3.4 Move read, write, flush, TLS authentication metadata, and graceful write-shutdown
...
- [x] 3.5 Implement idempotent nonparking terminal close that invalidates before one provider close
      ...
      cancellation.
- [x] 3.6 Replace borrowed-provider `withClient` with a consuming scoped convenience over
...
- [x] 4.1 Migrate every native/TLS integration caller to the owned APIs, delete callback-only
      compatibility paths, and verify the standard-library manifest and generated embedding expose
      only the new public surface.
- [x] 4.2 Update public reference documentation for owned authentication, explicit trust snapshot
...
- [x] 4.3 Consolidate runtime evidence into the existing TLS acceptance source, shared native corpus,
