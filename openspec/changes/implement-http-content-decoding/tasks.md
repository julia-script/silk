...
- [x] 1.3 Implement checked limit validation and owned-reservation calculation for fixed composition
...
- [x] 1.4 Implement Decode parsing across repeated `Content-Encoding` fields in wire order, including
      ...
      and excess depth.
- [x] 1.5 Implement Raw planning and response-context policy for semantic no-body responses, ordinary
...
- [x] 2.1 Implement the fixed four-slot codec representation and Effectful all-before-read
...
- [x] 2.2 Implement the private pure core combining `http_body.Decoder`, one fixed-capacity edge per
...
- [x] 2.3 Implement cursor scheduling with the consume/write/phase-or-return progress invariant and
...
- [x] 2.4 Enforce encoded bytes at deframed-body output, aggregate intermediate bytes at every
...
- [x] 2.5 Translate body, buffer/byte-I/O, inflate, Zstandard, trailing-zlib, arithmetic, and content
...
- [x] 2.6 Complete strict codec integration: HTTP deflate uses `Format.Zlib` only, gzip accepts its
...
- [x] 3.2 Implement Effectful `ContentReader.readSome` as the thin peek/core/consume/fill adapter with
      ...
      and failing reads.
- [x] 3.3 Implement non-I/O reader abandonment and partial-construction cleanup while leaving close
      ...
      Drop.
- [x] 3.4 Implement borrowed `RepresentationView`, untouched original header access, applied-plan and
...
- [x] 3.5 Implement opaque content completion and trailer access only after the body boundary, every
...
- [x] 4.3 Add one ticket-local structural acceptance file and one consolidated ordinary-source
