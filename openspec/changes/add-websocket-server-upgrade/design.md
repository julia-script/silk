...
- Use existing acceptance infrastructure for runtime and transport proofs; narrow structured analysis proves ownership. Keep the existing HTTP reference check at the frontend phase, which includes resolution, elaboration, and ownership diagnostics without unnecessary runtime realization. A focused standalone run verifies the complete WebSocket RFC example; the shared runtime fixture already guards its hash result, so the default suite does not add duplicate reference analysis. The ordinary runtime API makes no compile-time execution claim; StaticEvaluation is reserved for APIs explicitly available in that phase.
