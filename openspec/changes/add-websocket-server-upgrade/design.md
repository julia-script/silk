...
- Use existing acceptance infrastructure for runtime and transport proofs; narrow structured analysis proves ownership. Keep the existing HTTP reference check at the semantic frontend boundary with no retained runtime roots or editor tooling; resolution, elaboration, and ownership diagnostics are complete there. A focused standalone run verifies the complete WebSocket RFC example; the shared runtime fixture already guards its hash result, so the default suite does not add duplicate reference analysis. The ordinary runtime API makes no compile-time execution claim; StaticEvaluation is reserved for APIs explicitly available in that phase.
