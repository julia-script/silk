...
Equivalent accepted `Bytes` programs SHALL produce the same length, byte order, mutations, allocation
failures, ownership diagnostics, and cleanup behavior in MIR evaluation, native LLVM execution, and
direct Wasm execution. Tooling SHALL resolve the nominal type and its operations to canonical Silk
source.
