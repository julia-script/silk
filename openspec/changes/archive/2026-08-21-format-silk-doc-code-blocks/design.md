...
`@silklang/docgen` package: it normalizes `///` and `//!` tokens, parses CommonMark, and
...
Create `@silklang/formatter` with public `Formatter` and package-owned error actors. It depends on
`@silklang/compiler` for Silk syntax and layout and on `@silklang/docgen` for CommonMark
...
@silklang/compiler
        ▲       ▲
        │       │
documentation  formatter ◀── cli / lsp
        ▲          │
        └──────────┘
```
...
- Making the compiler depend on `@silklang/docgen` creates a cycle and violates the
...
Add a `CodeFence` actor to `@silklang/docgen`. It parses one compiler-owned `DocBlock`
...
3. Add `@silklang/formatter`, its public actors, package exports, error mapping, staged pipeline,
   and focused tests.
4. Move CLI and LSP whole-document adapters to the public formatter and add parity tests.
