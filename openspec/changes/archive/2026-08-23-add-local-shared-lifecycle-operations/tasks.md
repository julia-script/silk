...
- [x] 1.1 Add `sharedClone` and callback-shaped `sharedWithMut` to the sealed catalog with the exact
...
- [x] 1.2 Implement a target-bounded strong transition that compares before incrementing and returns
...
- [x] 1.3 Implement the `Available | Active` access transition so exactly one callback runs and conflict
...
- [x] 2.1 Create one position-restricted exclusive loan for the successful callback and end it before
      restoration and return; verify sequential access succeeds after the first callback completes.
- [x] 2.2 Reject direct and narrowed returned borrows with a diagnostic that relates the escape to the
      access boundary; verify diagnostic codes and both spans rather than message text.
      Evidence note: the direct source route pins `OWN0016` and both spans; narrowed borrow
      containment is pinned at the ownership-fact tier because `SEM0054`/`SEM0055` rejects a bare
      narrowed-reference result before ownership checking.
- [x] 2.3 Recursively reject the borrow inside generic results, aggregates, failure values, Effects,
      ...
      Evidence note: direct, Effect, and generic-aggregate routes are end-to-end source cases;
      narrowed, failure-value, and stored-callable constructors are classified at the focused
      ownership-fact tier because the existing `SEM0054`/`SEM0055` position rules intentionally
      reject those source shapes before ownership checking.
- [x] 2.4 Reject suspension while the access loan is live and verify no coroutine frame receives that
...
- [x] 3.1 Lower explicit and structured core drop to one opaque cleanup action: non-last decrement or
      last `T` cleanup followed by reclaim; verify ownership plans never recursively schedule `T` per handle.
- [x] 3.2 Verify clone and non-last drop during active access change only the strong count and cannot
...
- [x] 3.3 Add ownership fixtures for two-handle drop, two-frame typed-failure cleanup, acyclic nested
      cores, and a strong cycle; verify exact obligations and the specified cycle leak before engine work.

## 4. Verification

- [x] 4.1 Cover all four public shared/exclusive nested-access shapes at semantic and transition tiers;
      verify every nested operation selects conflict before forming another reference.
- [x] 4.2 Run `pnpm typecheck`, `pnpm exec biome check .`, and focused ownership/intrinsic tests; verify
