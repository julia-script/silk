...
`@silklang/compiler` currently ends at an immutable `Lexer.LexicalResult`: exact source bytes,
...
storage, filesystem, or compiler service. `@silklang/compiler` becomes a workspace runtime
