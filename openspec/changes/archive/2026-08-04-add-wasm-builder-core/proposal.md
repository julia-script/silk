# Add `@silk-effect/wasm` core builder
...
library in the same sense `@silk-effect/llvm` is, sharing its purpose (build module state, emit two
representations) but not its architecture. The design is WebAssembly-native.

## What Changes

- New package `@silk-effect/wasm` with subpath-exported actor modules, Effect-native errors, and no
...
- No changes to `@silk-effect/llvm`, `@silk-effect/compiler`, or any existing package.
