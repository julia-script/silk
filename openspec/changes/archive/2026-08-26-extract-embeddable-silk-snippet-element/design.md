...
else: no `@silk-effect/*` runtime dependency, no workspace import in `src/`. The bundle crosses
that boundary as data, not types: `Site.render` stays pure and takes the bundle *contents* as an
option, and only the command shell (`Cli.ts`) resolves `@silk-effect/snippet/bundle` to a file
path and reads it. The guard tests were deliberately and visibly amended — exactly the edit their
own comments anticipate — to allowlist `@silk-effect/snippet` as an opaque asset supplier and to
