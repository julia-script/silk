...
- No canonical _module_ identity yet — that arrives with `load-module-closure`; the sort key
...
1. **One concept module, per-phase collections.** `Diagnostic.ts` owns the model; phase results
   ...
   pure function the driver owns. _Alternative rejected:_ a single accumulated stream threaded
...
3. **Diagnostic identity is deterministic data, not object reference**: derived from originating
   ...
   makes causal links encodable and golden-testable. _Alternative rejected:_ incrementing global
...
5. **Structured reasons stay, and known related spans surface.** The parser's and semantic
   ...
   related-spans field is exercised from day one instead of shipping unused. _Alternative
   rejected:_ message-only diagnostics — loses machine-readable reason data the specs already
   promise.
