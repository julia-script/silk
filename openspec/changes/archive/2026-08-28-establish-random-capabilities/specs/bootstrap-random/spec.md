...
**Reason**: The unqualified `Random` identity now denotes secure byte filling rather than a seeded word stream.

**Migration**: Use `silk/insecure_random.InsecureRandom` for the former deterministic service, or implement the new secure `silk/random.Random` exact-fill contract.

### Requirement: Xoshiro256StarStar has a stable seeded sequence

**Reason**: The algorithm remains supported but moves under the explicit insecure capability.

**Migration**: Import `silk/insecure_random` and use its unchanged `Xoshiro256StarStar` and `seeded` API.

### Requirement: Boolean sampling consumes one word deterministically

**Reason**: Secure and insecure Boolean operations now belong to distinct capability contracts.

**Migration**: Use `Random.nextBool` for secure data or `InsecureRandom.nextBool` for deterministic data.

### Requirement: Bounded sampling is unbiased and total at zero

**Reason**: Secure and insecure bounded operations now belong to distinct capability contracts.

**Migration**: Use `Random.below` for secure data or `InsecureRandom.below` for deterministic data.

### Requirement: Byte filling has a stable word-to-byte mapping

**Reason**: Secure `Random` now owns byte filling directly, while the old word-derived mapping moves under `InsecureRandom`.

**Migration**: Use `Random.fillBytes` for secure data or `InsecureRandom.fillBytes` for the former deterministic mapping.

### Requirement: Seeded random is explicitly non-cryptographic

**Reason**: The warning remains true but now belongs specifically to the renamed `InsecureRandom` capability.

**Migration**: Import `silk/insecure_random`; never use it where the new secure `Random` contract is required.
