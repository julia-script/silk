...
Copy only when every member is Copy and cleanup-free; otherwise it SHALL remain one move-only owner.
Borrowed values SHALL NOT be stored as union members, and cleanup SHALL act on exactly the active
payload once.
