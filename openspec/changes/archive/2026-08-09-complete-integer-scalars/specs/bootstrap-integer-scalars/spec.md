...
Integer literals SHALL retain exact magnitude until typed. An immediate integer context SHALL select a type only when representable; an unconstrained integer SHALL default to `i32`. Range rejection MUST occur before MIR lowering without JavaScript-number rounding.
