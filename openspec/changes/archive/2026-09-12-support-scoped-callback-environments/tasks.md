...
- [x] 4.3 Publish the coherent focused commit range only with publication authorization and run broad delivery checks in CI, including `pnpm check` and package-content validation when applicable; record exact revisions and outcomes and hand integration instructions to the paused implementer without merging or completing JUL-188.
