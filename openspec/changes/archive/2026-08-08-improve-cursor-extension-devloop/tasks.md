...
- [x] 1.1 Add a Node install script under `packages/vscode` that builds `@silk-lang/lsp` and `silk-language`, then `ln -sfn` the package folder to `~/.cursor/extensions/silk-effect.silk-language-0.0.0`
...
- [x] 2.1 Add `.vscode/tasks.json` entries to build (and optionally watch) `@silk-lang/lsp` and `silk-language`, including a compound pre-launch build task
...
- [x] 4.1 Run `pnpm typecheck`, package tests for `@silk-lang/language`, and `pnpm exec biome check` on touched paths
