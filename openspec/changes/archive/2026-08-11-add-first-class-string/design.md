...
*Alternative considered:* represent text as nominal `silk/string.String` or recognize that
...
*Alternative considered:* brand `&[u8]` only in tooling. Rejected because the brand would be lost by
...
*Alternative considered:* one `String` type with static, borrowed, and owned storage variants.
...
*Alternative considered:* permanently specify `{ pointer, usize }`. Rejected because it would make
...
*Alternative considered:* lower equality to a compiler-generated byte loop. Rejected because that
