...
- [x] 1.1 Add opaque affine local `Intrinsic.Wake`, consuming synchronous `wake`, and effectful unit-
...
- [x] 1.2 Extend external-park package plans with one stable wake-control cell and fixed endpoint only
      when statically reachable; verify park, wake, notification, and generation reuse contain no
      allocator access or failure edge.
- [x] 1.3 Add verified MIR state/authority transitions for Registering, Latched, Dormant, Notifying,
...
- [x] 2.1 Implement park registration with the generation's sole Wake and retain returned `G` plus
...
- [x] 2.2 Implement wake-during-registration latching and gate notification on complete `onSuspend`
      return; verify the Execution relinquishes once and notification cannot observe Running state.
- [x] 2.3 Cover `onSuspend` destruction after a latched Wake; verify cancellation suppresses endpoint
      invocation and cleans continuation, endpoint, and guard values exactly once.
- [x] 2.4 Implement Wake-after-dormant one-shot notification; verify the endpoint runs at most once,
      wake never drives inline, and safe source cannot signal the generation twice.

## 3. Notification, Cancellation, and Reclamation

- [x] 3.1 End cell mutation, enter Notifying, and take an invocation retain before calling `R(&O)`;
      verify the Execution becomes Eligible only after ordinary callback return.
- [x] 3.2 Implement the defined fatal trap for indirect drive while Notifying; verify no endpoint
      replacement, continuation progress, or drive callback occurs.
- [x] 3.3 Implement reentrant destruction as DestroyPending and defer endpoint cleanup/package release
      until callback return; verify borrowed `O` and `R` remain live and the Execution never becomes
      Eligible afterward.
- [x] 3.4 Cancel before dormant cleanup and retain the indivisible Allocation behind every external
...
- [x] 3.5 Reinitialize the stable cell only after the prior generation Wake and transients end; verify
      repeated park/wake/resume cycles reuse storage without aliasing generations.
- [x] 3.6 Preserve local affinity for Execution and Wake and add same-thread source extraction tests;
...
- [x] 4.1 Add ordinary Deferred-shaped and timer-shaped registration fixtures that extract Wake under
...
- [x] 4.2 Audit intrinsic and phase inventories for explicit cancel/destroy, Scheduler tokens, payload
      transport, actor-name privilege, hidden allocation, and root policy; verify none is introduced.
- [x] 4.3 Run focused wake-order, ownership, cleanup, reentrancy, generation, local-affinity, and
