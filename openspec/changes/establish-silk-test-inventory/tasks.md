...
- [ ] 5.3 Run focused syntax, semantic, closure, evaluation, cleanup, and privilege tests, then `pnpm typecheck`, `pnpm exec biome check .`, `pnpm test`, `pnpm check`, and `pnpm release:candidate`; record every exact result and identify any pre-existing failure before handing off the standard-library slice.
