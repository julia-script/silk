...
an initialized slot, and inspect the logical count. The compiler SHALL validate canonical
type/layout provenance, checked index ordering, slot non-escape, exclusive owner liveness, and legal
element operations. Unsafe Silk code remains responsible for the runtime initializedness and
aliasing invariants; the runtime and compiler MUST NOT add a collection-shaped initialization
bitmap.
...
- **WHEN** unsafe code takes a value from a slot that its own runtime state has not initialized
