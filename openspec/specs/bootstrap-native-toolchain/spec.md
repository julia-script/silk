...
Object emission SHALL invoke the caller-pinned external Clang with `-c` and structured arguments
— never a shell command string — over the backend's bitcode. Optimization profiles are fixed:
debug is `-O0` with debug metadata, release is `-O2` without debug metadata, and
release-with-debug is `-O2` with line information. There is no configurable pass pipeline, and a
process failure SHALL surface as data retaining the exact command, arguments, exit status, and
process output rather than throwing.
...
- **THEN** a non-empty relocatable object file exists at the scope-owned path and the outcome records the exact command and arguments issued
...
- **WHEN** the three profiles' commands are planned for the same input
- **THEN** debug plans `-O0` with `-g`, release plans `-O2` without `-g`, and release-with-debug plans `-O2` with line information — and nothing else varies
...
The `NativeLinker` service SHALL validate that its inputs exist and are target-compatible,
combine the program object with the selected runtime objects and approved system libraries,
invoke the pinned Clang driver with structured arguments, and write the executable to the
requested durable destination. On failure the outcome SHALL retain process output, exit status,
and command provenance as data.

#### Scenario: Link a runnable executable

- **WHEN** the `ClangLinker` links the program object with the compiled runtime shim
