...
program as a lazy consuming Effect, bind an owned Scheduler provider for that program, create the
root Execution as the scheduler's first task, and drive tasks privately until the root terminates.
The compiler MUST NOT select, construct, recognize, or enter a Scheduler implicitly.

#### Scenario: Execute a scheduler-owned root

- **WHEN** ordinary `main` constructs a `LocalScheduler` and passes a lazy program to `execute`
- **THEN** the program begins only after `execute` creates and publishes its root task
...
- **WHEN** root packaging or scheduler-owned storage cannot be funded before publication
- **THEN** `execute` raises `Allocator.OutOfMemoryError` and leaves no live root or child task
...
`Fiber.forkChild` SHALL accept one consuming lazy Effect whose unresolved runtime requirement is at
most the child Scheduler provider, and SHALL return an affine `Fiber<A, E>`. It SHALL fund the
completion state, child Scheduler client, readiness endpoint, task identity, and child Execution
before making the child observable. Publication failure SHALL raise its exact typed scheduling or
allocation failure and SHALL not return a Fiber or leave a runnable child. A successfully published
child MUST NOT begin its first activation before `forkChild` returns its Fiber to the parent.
...
- **WHEN** any required child allocation, task identity, or task-store publication fails
- **THEN** `forkChild` raises the exact failure and ordinary cleanup destroys every prepared but unpublished value
...
the ready tail without allocation, and permit it to resume after tasks already ready.
...
descendant, release every scheduler-owned task, mailbox, readiness node, completion authority, and
container entry, and restore the provider to an empty reusable state. Fatal traps remain governed
by the language's existing trap cleanup boundary.

#### Scenario: Clean descendants after root completion

- **WHEN** the root reaches any typed terminal outcome with unfinished descendants
- **THEN** `execute` performs structured cancellation and returns with no task retained by the scheduler
...
- **THEN** the second program starts with a fresh empty task tree and cannot observe stale readiness from the first run
