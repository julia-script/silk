...
`string` SHALL be an immutable UTF-8 value whose references, byte views, moves, equality, length,
calls, returns, and lexical lifetimes obey the ordinary type and ownership rules. A `string` and
`&[u8]` SHALL remain distinct logical types even when they name identical valid UTF-8 bytes.
...
Standard-library traversal SHALL decode validated UTF-8 into `char` through a checked scalar
conversion. Invalid scalar values SHALL be rejected through the conversion's declared boundary,
and no implicit truncation or locale behavior SHALL occur.
